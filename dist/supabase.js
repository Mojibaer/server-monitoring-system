...
exports.ensureSupabaseRunning = ensureSupabaseRunning;
exports.ensureMonitoringSchema = ensureMonitoringSchema;
...
const supabaseDir = node_path_1.default.join(process.cwd(), "supabase");
...
function runDocker(args, input) {
    if (process.platform === "win32") {
        return (0, node_child_process_1.spawnSync)("cmd.exe", ["/d", "/s", "/c", ["docker", ...args].map(quoteForCmd).join(" ")], {
            cwd: supabaseDir,
            input,
            stdio: input ? ["pipe", "inherit", "inherit"] : "inherit"
        });
    }
    return (0, node_child_process_1.spawnSync)("docker", args, {
        cwd: supabaseDir,
        input,
        stdio: input ? ["pipe", "inherit", "inherit"] : "inherit"
    });
...
async function waitForSupabase(throwOnTimeout) {
